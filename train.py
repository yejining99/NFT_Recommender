...
from Model_attention import MMMO, GRAPH
import pickle
...
class Net:
    def __init__(self, args):
        ...
        hyperparameters = {'model_name': args.model_name, 'data_path': args.data_path, 'learning_rate': args.l_r, 
                            'dim_latent': args.dim_latent, 'reg_parm': args.reg_parm, 'loss_alpha': args.loss_alpha, 'optimizer': args.optim}
        ...
        elif self.optim == 'Adam_SWA':
            self.base_optimizer = torch.optim.Adam(self.model.parameters(), lr=self.learning_rate)
            self.optimizer = SWA(self.base_optimizer, swa_start=5, swa_freq=3, swa_lr=0.001)
    ...
    def run(self):
        ...
        for epoch in range(self.num_epoch):
            ...
            if self.optim == 'Adam_SWA' or self.optim == 'SGD_SWA':
            ...
            with torch.no_grad():
                ...
                test_ndcg_score = test_ndcg_score[1]
            ...
            test_ndcgs.append(test_ndcg_score)

        print('Best Recall:{0:.4f} NDCG:{1:.4f}'.format(best_recall, best_ndcg_score))
        
        # save the recalls, test_recalls into plot
        plt.plot(recalls, label='Valid Recall')
        plt.plot(test_recalls, label='Test Recall')
        plt.legend
        plt.title(f'{self.optim} Recall')
        plt.savefig(args.PATH_weight_save + args.model_name + args.data_path[i:-1] + self.number + '_Recall' + '.png')
        plt.close()

        # save the ndcgs, test_ndcgs into plot
        plt.plot(ndcgs, label='Valid NDCG')
        plt.plot(test_ndcgs, label='Test NDCG')
        plt.legend()
        plt.title(f'{self.optim} NDCG')
        plt.savefig(args.PATH_weight_save + args.model_name + args.data_path[i:-1] + self.number + '_NDCG' + '.png')
        plt.close()

        # save losses, BPR_losses, Price_losses, recalls, ndcgs as pickle file
        result = {f'losses': losses, f'BPR_losses': BPR_losses, f'Price_losses': Price_losses, f'recalls': recalls, f'ndcgs': ndcgs, f'test_recalls':test_recalls, f'test_ndcgs':test_ndcgs, f'optimizer':self.optim}
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', default='dataset/bayc/', help='Dataset path')
    parser.add_argument('--PATH_weight_load', default=None, help='Loading weight filename.')
    parser.add_argument('--PATH_weight_save', default='model/', help='Writing weight filename.')
